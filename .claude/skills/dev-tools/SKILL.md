...
```
.claude\skills\dev-tools\scripts\grep.ps1 -Pattern "TODO|FIXME"
.claude\skills\dev-tools\scripts\grep.ps1 -Pattern "class\s+\w+" -Include "*.cs" -Context 2
```
Params: `-Pattern` (required), `-Path`, `-Include`, `-Context`, `-CaseSensitive`, `-MaxResults`

### `find-files.ps1` — Find files by name pattern
```
.claude\skills\dev-tools\scripts\find-files.ps1 -Name "*.test.ts"
.claude\skills\dev-tools\scripts\find-files.ps1 -Name "Controller" -Regex
.claude\skills\dev-tools\scripts\find-files.ps1 -Name "components" -DirectoriesOnly
```
Params: `-Name` (required), `-Path`, `-Regex`, `-DirectoriesOnly`, `-MaxResults`

### `tree.ps1` — Directory tree with depth control
```
.claude\skills\dev-tools\scripts\tree.ps1
.claude\skills\dev-tools\scripts\tree.ps1 -Path "src" -Depth 4 -ShowFiles
```
...
.claude\skills\dev-tools\scripts\git-diff.ps1 -Staged -NameOnly
.claude\skills\dev-tools\scripts\git-diff.ps1 -Ref1 "main" -Stat
```
...
```
.claude\skills\dev-tools\scripts\remove-files.ps1 -Path "temp.log"
.claude\skills\dev-tools\scripts\remove-files.ps1 -Path "dist","coverage" -Recurse -Force
.claude\skills\dev-tools\scripts\remove-files.ps1 -Path "src\old-module" -Recurse
```
Params: `-Path` (required, accepts array), `-Recurse`, `-Force`, `-Quiet`

> Dry-run by default. BLOCKS any path outside the working directory, parent traversal (`..\`), system directories, `.git`, `.claude`, and `node_modules`. Use this instead of `Remove-Item` or the `remove-files` tool.
