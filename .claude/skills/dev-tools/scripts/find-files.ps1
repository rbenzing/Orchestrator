...
if ($ExtraArgs) {
    ...
    Write-Host "  Usage: .claude\skills\dev-tools\scripts\find-files.ps1 -Name ""*.ts"" -Path ""src""" -ForegroundColor Yellow
}
...
if ($count -ge $MaxResults) { Write-Host "`n--- Stopped at $MaxResults results ---" -ForegroundColor Yellow }
Write-Host "`nFound: $count" -ForegroundColor Green
