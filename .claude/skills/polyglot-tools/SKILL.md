...
```
.claude\skills\polyglot-tools\scripts\python-run.ps1 -ScriptPath "main.py"
.claude\skills\polyglot-tools\scripts\python-run.ps1 -Module "pytest" -ProjectPath "tests" -PassThruArgs "-v","--tb=short"
```
...
.claude\skills\polyglot-tools\scripts\pip-install.ps1 -RequirementsFile "requirements.txt" -ProjectPath "backend"
```
Params: `-Packages` OR `-RequirementsFile` (one required), `-ProjectPath`, `-Upgrade`, `-PassThruArgs`

### `poetry-run.ps1` — Run Poetry commands
```
.claude\skills\polyglot-tools\scripts\poetry-run.ps1 -Command "install" -ProjectPath "backend"
.claude\skills\polyglot-tools\scripts\poetry-run.ps1 -Command "run" -PassThruArgs "pytest","-v"
```
...
```
.claude\skills\polyglot-tools\scripts\cargo-run.ps1 -Command "build" -ProjectPath "rust-lib"
.claude\skills\polyglot-tools\scripts\cargo-run.ps1 -Command "test" -PassThruArgs "--release"
```
...
.claude\skills\polyglot-tools\scripts\ruby-run.ps1 -BundleCommand "exec" -PassThruArgs "rails","server"
```
