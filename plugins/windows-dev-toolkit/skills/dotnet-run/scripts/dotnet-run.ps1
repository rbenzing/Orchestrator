...
.EXAMPLE
    .claude\skills\dotnet-run\scripts\dotnet-run.ps1 -ProjectPath "src\MyApi"
.EXAMPLE
    .claude\skills\dotnet-run\scripts\dotnet-run.ps1 -ProjectPath "src\MyApi" -LaunchProfile "Development"
#>
[CmdletBinding()]
param(
...
Write-Host "Running .NET project in: $resolved"
Write-Host "Configuration: $Configuration"
if ($LaunchProfile) { Write-Host "Launch profile: $LaunchProfile" }
...
try {
    ...
    & dotnet @runArgs 2>&1
    $code = $LASTEXITCODE
    if ($code -eq 0) { Write-Host "RUN COMPLETED (exit 0)" }
    else { Write-Host "RUN FAILED (exit $code)" }
    exit $code
