...
.EXAMPLE
    .claude\skills\ng-test\scripts\run-tests.ps1 -ProjectPath "ClientApp"
.EXAMPLE
    .claude\skills\ng-test\scripts\run-tests.ps1 -ProjectPath "ClientApp" -LegacyOpenSSL -Headless -NoWatch
.EXAMPLE
    .claude\skills\ng-test\scripts\run-tests.ps1 -ProjectPath "." -Include "src/app/components" -CodeCoverage
#>
[CmdletBinding()]
param(
...
Write-Host "Running Angular tests in: $resolved"
...
try {
    ...
    & npm @npmArgs 2>&1
    $code = $LASTEXITCODE
    if ($code -eq 0) { Write-Host "TESTS PASSED (exit 0)" }
    else { Write-Host "TESTS FAILED (exit $code)" }
    exit $code
} finally {
