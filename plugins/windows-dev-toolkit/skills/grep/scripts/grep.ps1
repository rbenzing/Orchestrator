...
.EXAMPLE
    .claude\skills\grep\scripts\grep.ps1 -Pattern "TODO|FIXME"
.EXAMPLE
    .claude\skills\grep\scripts\grep.ps1 -Pattern "class\s+\w+" -Include "*.cs" -Context 2
#>
[CmdletBinding()]
param(
...
if ($ExtraArgs) {
    ...
    Write-Host "Usage: .claude\skills\grep\scripts\grep.ps1 -Pattern ""TODO"" -Include ""*.ts"""
}

$ExcludeDirs = @('.git','node_modules','bin','obj','dist','build','vendor',
                  '__pycache__','.vs','.idea','packages','TestResults','.next','.claude')
...
foreach ($file in $files) {
    ...
    for ($i = 0; $i -lt $lines.Count; $i++) {
        if ([regex]::IsMatch($lines[$i], $Pattern, $regexOpts)) {
            ...
            $ln = $i + 1
            Write-Host "${rel}:${ln}: $($lines[$i].TrimEnd())"
            if ($Context -gt 0) {
                $s = [Math]::Max(0, $i - $Context); $e = [Math]::Min($lines.Count - 1, $i + $Context)
                for ($j = $s; $j -le $e; $j++) {
                    if ($j -ne $i) { Write-Host "${rel}:$($j+1): $($lines[$j].TrimEnd())" }
                }
            }
        }
    }
}
Write-Host "`nTotal matches: $matchCount"