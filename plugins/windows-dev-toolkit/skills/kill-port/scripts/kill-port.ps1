...
.EXAMPLE
    .claude\skills\kill-port\scripts\kill-port.ps1 -Port 3000
.EXAMPLE
    .claude\skills\kill-port\scripts\kill-port.ps1 -Port 3000 -Force
.EXAMPLE
    .claude\skills\kill-port\scripts\kill-port.ps1 -ProcessId 12345 -Force
#>
...
if ($PSCmdlet.ParameterSetName -eq 'ByPid') {
    Write-Host "Looking up PID $ProcessId..."
    try { $proc = Get-Process -Id $ProcessId -ErrorAction Stop }
    catch { Write-Host "No process with PID $ProcessId."; exit 1 }
    $mem = [math]::Round($proc.WorkingSet64 / 1MB, 1)
    Write-Host "Found: $($proc.ProcessName) (PID $($proc.Id), ${mem}MB)"
    if (Test-Protected $proc) { Write-Host "BLOCKED - critical process."; exit 1 }
    if (-not $Force) { Write-Host "DRY RUN - add -Force to kill."; exit 0 }
    Stop-Process -Id $proc.Id -Force; Write-Host "Stopped."; exit 0
}

# Port mode
Write-Host "Checking port $Port..."
try { $conns = Get-NetTCPConnection -LocalPort $Port -ErrorAction SilentlyContinue }
catch { Write-Host "Port $Port is free."; exit 0 }
if (-not $conns) { Write-Host "Port $Port is free."; exit 0 }
...
if ($procs.Count -eq 0) { Write-Host "Port bound but process exited."; exit 0 }

if ($ProcessName) {
    $filtered = $procs | Where-Object { $_.ProcessName -like $ProcessName }
    if ($filtered.Count -eq 0) {
        Write-Host "Port $Port in use but not by '$ProcessName'."; exit 0
    }
    $procs = @($filtered)
}

Write-Host "Found $($procs.Count) process(es) on port ${Port}:"
foreach ($p in $procs) { Write-Host "  PID $($p.Id): $($p.ProcessName) ($([math]::Round($p.WorkingSet64/1MB,1))MB)" }

if (-not $Force) { Write-Host "`n  DRY RUN - add -Force to kill."; exit 0 }

$killed = 0
foreach ($p in $procs) {
    if (Test-Protected $p) { Write-Host "SKIPPED - $($p.ProcessName) is protected."; continue }
    Stop-Process -Id $p.Id -Force; $killed++; Write-Host "Stopped PID $($p.Id)."
}
Write-Host "`n  Done - stopped $killed process(es)."