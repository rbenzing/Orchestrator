...
.EXAMPLE
    .claude\skills\get-compact-diff\scripts\get-compact-diff.ps1
.EXAMPLE
    .claude\skills\get-compact-diff\scripts\get-compact-diff.ps1 `
...
Write-Host "[get-compact-diff] Generating compact diff..."
...
Write-Host "Raw diff: $totalLines lines"
...
$baseLabel = if ($Staged) { "staged (index)" } else { $BaseBranch }
...
if ($totalLines -le $MaxLines) {
    ...
    Write-Host "[!] Diff truncated to $MaxLines lines (was $totalLines)."
}
...
if ($OutputFile) {
    ...
    Write-Host "Diff saved: $OutputFile"
}
