...
.EXAMPLE
    .claude\skills\git-diff\scripts\git-diff.ps1
.EXAMPLE
    .claude\skills\git-diff\scripts\git-diff.ps1 -Staged
.EXAMPLE
    .claude\skills\git-diff\scripts\git-diff.ps1 -Ref1 "main" -Ref2 "feature/login"
.EXAMPLE
    .claude\skills\git-diff\scripts\git-diff.ps1 -Ref1 "HEAD~5" -Stat
#>
[CmdletBinding()]
param(
    [switch]$Staged,
...
try {
    ...
    if ($Ref1 -and $Ref2) {
        $args_ += "$Ref1..$Ref2"
        Write-Host "Diff: $Ref1..$Ref2"
    } elseif ($Ref1) {
        $args_ += "$Ref1..HEAD"
        Write-Host "Diff: $Ref1..HEAD"
    } elseif ($Staged) {
        $args_ += "--cached"
        Write-Host "Diff: staged changes"
    } else {
        Write-Host "Diff: unstaged changes"
    }

    if ($FilePath) {
        $args_ += "--"
        $args_ += $FilePath
        Write-Host "Path filter: $FilePath"
    }

    Write-Host "> git diff $($args_ -join ' ')"
    & git diff @args_
