...
.EXAMPLE
    .claude\skills\extract-symbols\scripts\extract-symbols.ps1 `
...
Write-Host "[extract-symbols] $FilePath  ($totalLines lines total)"
...
foreach ($symbol in $Symbols) {
    ...
    if ($startLine -eq -1) {
        Write-Host "[!] Symbol '$symbol' not found in $FilePath"
        continue
    }
    ...
    for ($i = $startLine; $i -lt $totalLines; $i++) {
        ...
        if (-not $foundOpenBrace -and $i -gt $startLine) {
            $baseIndent = if ($lines[$startLine] -match '^(\s*)') { $Matches[1].Length } else { 0 }
            $lineIndent = if ($lines[$i]        -match '^(\s*)') { $Matches[1].Length } else { 0 }
    ...
    Write-Host "Lines $($from+1)-$($to+1) of $totalLines:"
