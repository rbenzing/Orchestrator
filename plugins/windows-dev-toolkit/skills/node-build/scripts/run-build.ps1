...
.EXAMPLE
    .claude\skills\node-build\scripts\run-build.ps1 -ProjectPath "ClientApp"
.EXAMPLE
    .claude\skills\node-build\scripts\run-build.ps1 -ProjectPath "src\app" -SourceMap
.EXAMPLE
    .claude\skills\node-build\scripts\run-build.ps1 -ProjectPath "." -ScriptName "build:prod"
.EXAMPLE
    .claude\skills\node-build\scripts\run-build.ps1 -ProjectPath "ClientApp" -PassThruArgs "--stats"
#>
[CmdletBinding()]
param(
...
Write-Host "Building project in: $resolved"
Write-Host "Script: npm run $ScriptName"
if ($SourceMap) { Write-Host "Source maps: enabled" }
