...
.EXAMPLE
    .claude\skills\remove-files\scripts\remove-files.ps1 -Path "temp.log"
.EXAMPLE
    .claude\skills\remove-files\scripts\remove-files.ps1 -Path "dist","coverage" -Recurse -Force
.EXAMPLE
    .claude\skills\remove-files\scripts\remove-files.ps1 -Path "src\old-module" -Recurse
#>
[CmdletBinding()]
param(
...
$BlockedNames = @('.git', '.claude', 'node_modules')
...
foreach ($p in $Path) {
    ...
    if ($isDir -and -not $Recurse) {
        ...
        if ($children.Count -gt 0) {
            Write-Host "BLOCKED: $p is a non-empty directory. Add -Recurse to confirm."
            $totalErrors++
            continue
        }
    }
    ...
    if (-not $Force) {
        if (-not $Quiet) { Write-Host "WOULD REMOVE: $p -- $label" }
        $totalRemoved++
        continue
    }
    ...
    try {
        ...
        if (-not $Quiet) { Write-Host "REMOVED: $p -- $label" }
        $totalRemoved++
    } catch {
...
Write-Host "$mode -- $totalRemoved item(s) processed, $totalSkipped skipped, $totalErrors error(s)"
if (-not $Force -and $totalRemoved -gt 0) {
    Write-Host "Add -Force to actually delete."
}