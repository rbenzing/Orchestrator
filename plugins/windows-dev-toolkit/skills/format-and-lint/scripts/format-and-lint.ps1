...
.EXAMPLE
    .claude\skills\format-and-lint\scripts\format-and-lint.ps1
.EXAMPLE
    .claude\skills\format-and-lint\scripts\format-and-lint.ps1 -Fix:$false
.EXAMPLE
    .claude\skills\format-and-lint\scripts\format-and-lint.ps1 -Paths "src/auth","src/api"
#>
[CmdletBinding()]
param(
...
Write-Host "[format-and-lint] Detecting project type..."
...
if (Test-Path $pkgJson) {
    ...
    if ($prettierCfg) {
        Write-Host "[prettier] Running on $pathArg..."
        if ($Fix) {
            $out = & npm exec -- prettier --write $pathArg 2>&1
        } else {
            $out = & npm exec -- prettier --check $pathArg 2>&1
        }
    ...
    $eslintCfg = @(".eslintrc", ".eslintrc.json", ".eslintrc.js", ".eslintrc.cjs", "eslint.config.js") |
        Where-Object { Test-Path (Join-Path $Root $_) } | Select-Object -First 1
    if ($eslintCfg) {
        Write-Host "[eslint] Running on $pathArg..."
        $eslintArgs = @($pathArg, "--ext", ".ts,.tsx,.js,.jsx")
        if ($Fix) { $eslintArgs += "--fix" }
        $out = & npm exec -- eslint @eslintArgs 2>&1
...
if ($csprojFiles) {
    Write-Host "[dotnet format] Running..."
    $fmtArgs = @("format")
...
if ($pyFiles) {
    ...
    & where.exe ruff 2>&1 | Out-Null
    if ($LASTEXITCODE -eq 0) {
        Write-Host "[ruff] Running on $pathArg..."
        ...
        & where.exe black 2>&1 | Out-Null
        if ($LASTEXITCODE -eq 0) {
            Write-Host "[black] Running on $pathArg..."
...
if ($results.Count -eq 0) {
    Write-Host "[!] No supported formatter/linter config detected. Skipping."
    Write-Host "    Supported: prettier, eslint, dotnet format, ruff, black"
} else {
    $failed = $results | Where-Object { $_.ExitCode -ne 0 }
    if ($failed.Count -eq 0) {
        Write-Host "[format-and-lint] All tools passed. $($results.Count) tool(s) ran."
    } else {
        Write-Host "[format-and-lint] $($failed.Count) tool(s) reported issues:"
        foreach ($f in $failed) {
            Write-Host "  - $($f.Tool) (exit $($f.ExitCode))"
        }
        Write-Host "Resolve lint errors before invoking the Code Reviewer agent."
    }
}
