...
```powershell
${CLAUDE_PLUGIN_ROOT}\skills\dev-tools\scripts\grep.ps1 -Pattern "TODO|FIXME"
${CLAUDE_PLUGIN_ROOT}\skills\dev-tools\scripts\grep.ps1 -Pattern "class\s+\w+" -Include "*.cs" -Context 2
```
...
${CLAUDE_PLUGIN_ROOT}\skills\dev-tools\scripts\find-files.ps1 -Name "Controller" -Regex
${CLAUDE_PLUGIN_ROOT}\skills\dev-tools\scripts\find-files.ps1 -Name "components" -DirectoriesOnly
```
Params: `-Name` (required), `-Path`, `-Regex`, `-DirectoriesOnly`, `-MaxResults`

### `tree.ps1` — Directory tree with depth control
```powershell
${CLAUDE_PLUGIN_ROOT}\skills\dev-tools\scripts\tree.ps1
${CLAUDE_PLUGIN_ROOT}\skills\dev-tools\scripts\tree.ps1 -Path "src" -Depth 4 -ShowFiles
```
Params: `-Path`, `-Depth`, `-ShowFiles`

### `git-summary.ps1` — Branch, status, and recent commits
```powershell
${CLAUDE_PLUGIN_ROOT}\skills\dev-tools\scripts\git-summary.ps1
${CLAUDE_PLUGIN_ROOT}\skills\dev-tools\scripts\git-summary.ps1 -LogCount 20 -ShowStash
```
Params: `-Path`, `-LogCount`, `-ShowStash`

### `git-diff.ps1` — Staged, unstaged, or ref-based diffs
```powershell
${CLAUDE_PLUGIN_ROOT}\skills\dev-tools\scripts\git-diff.ps1
${CLAUDE_PLUGIN_ROOT}\skills\dev-tools\scripts\git-diff.ps1 -Staged -NameOnly
${CLAUDE_PLUGIN_ROOT}\skills\dev-tools\scripts\git-diff.ps1 -Ref1 "main" -Stat
```
Params: `-Staged`, `-Ref1`, `-Ref2`, `-Path`, `-FilePath`, `-Stat`, `-NameOnly`

### `kill-port.ps1` — Kill process by TCP port or PID
```powershell
${CLAUDE_PLUGIN_ROOT}\skills\dev-tools\scripts\kill-port.ps1 -Port 3000
${CLAUDE_PLUGIN_ROOT}\skills\dev-tools\scripts\kill-port.ps1 -Port 3000 -Force
${CLAUDE_PLUGIN_ROOT}\skills\dev-tools\scripts\kill-port.ps1 -ProcessId 12345 -Force
```
...
```powershell
${CLAUDE_PLUGIN_ROOT}\skills\dev-tools\scripts\remove-files.ps1 -Path "temp.log"
${CLAUDE_PLUGIN_ROOT}\skills\dev-tools\scripts\remove-files.ps1 -Path "dist","coverage" -Recurse -Force
${CLAUDE_PLUGIN_ROOT}\skills\dev-tools\scripts\remove-files.ps1 -Path "src\old-module" -Recurse
```
Params: `-Path` (required, accepts array), `-Recurse`, `-Force`, `-Quiet`

> Dry-run by default. BLOCKS any path outside the working directory, parent traversal (`..\`), system directories, `.git`, `.claude`, and `node_modules`.
