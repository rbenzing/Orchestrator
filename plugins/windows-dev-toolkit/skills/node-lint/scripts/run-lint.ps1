...
.EXAMPLE
    .claude\skills\node-lint\scripts\run-lint.ps1 -ProjectPath "ClientApp"
.EXAMPLE
    .claude\skills\node-lint\scripts\run-lint.ps1 -ProjectPath "src\app" -Fix
.EXAMPLE
    .claude\skills\node-lint\scripts\run-lint.ps1 -ProjectPath "." -Quiet -MaxWarnings 0
.EXAMPLE
    .claude\skills\node-lint\scripts\run-lint.ps1 -ProjectPath "ClientApp" -Files "src/**/*.ts" -Fix
#>
[CmdletBinding()]
param(
...
Write-Host "Running lint in: $resolved"
...
try {
    ...
    & npm @npmArgs 2>&1
    $code = $LASTEXITCODE
    if ($code -eq 0) { Write-Host "LINT PASSED (exit 0)" }
    else { Write-Host "LINT FAILED (exit $code)" }
    exit $code
} finally { Pop-Location }