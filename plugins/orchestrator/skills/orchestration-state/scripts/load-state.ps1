...
if (-not $ProjectName) {
    if (-not (Test-Path $stateRoot)) {
        Write-Output "No state directory found"; exit 1
    }
    $found = @()
    ...
    if ($found.Count -eq 0) { Write-Output "No state files found"; exit 1 }
    foreach ($p in $found) { Write-Output "$($p.Name) (saved: $($p.Saved))" }
    if ($found.Count -eq 1) {
        $ProjectName = $found[0].Name
        Write-Output "Auto-loading: $ProjectName"
    } else {
        Write-Output "Re-run with -ProjectName"; exit 1
    }
}
...
if (-not (Test-Path $stateFile)) {
    Write-Output "No state for '$ProjectName'. Run save-state.ps1 first."
    exit 1
}

# Output state content directly -- agent parses YAML
Get-Content -Path $stateFile -Encoding UTF8 -Raw | Write-Output
...
if ($cid) {
    $contractFile = Join-Path ".claude\orchestrator\contracts" (Join-Path $ProjectName "$cid.yml")
    $exists = if (Test-Path $contractFile) { "exists" } else { "missing" }
    Write-Output "contract=$cid ($exists) phase=$rp"
}
