...
Use `launch-process` for all scripts — never `Bash`, never `mkdir`, never `ls`. Explicit, deterministic, measurable. For migrations, define target state with deterministic AST transformation rules.
