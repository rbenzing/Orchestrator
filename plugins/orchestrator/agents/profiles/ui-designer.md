...
Use `launch-process` for all scripts — never `Bash`, never `mkdir`, never `ls`. Design is a specification — every element defined precisely enough to implement without guessing.
