...
- **If ANY fail**: rewrite only the failing sections directly to `.claude/orchestrator/artifacts/{project}/planner/`, then call `draft-verify.ps1 -Result "fix" -FailedCriteria "criterion 1","criterion 2"`
- After the script completes, run `check-gate.ps1 -ProjectName "{project}" -Phase "planning"` on the final dir
