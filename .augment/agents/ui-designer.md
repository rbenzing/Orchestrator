...
| When | Script | Command |
|---|---|---|
| **Phase start** — check upstream artifacts | `artifact-status.ps1` | `.augment\skills\orchestration-artifacts\scripts\artifact-status.ps1 -ProjectName "{project}"` |
| **Before handoff** — validate quality gate | `check-gate.ps1` | `.augment\skills\orchestration-artifacts\scripts\check-gate.ps1 -ProjectName "{project}" -Phase "ui-design"` |
| **At handoff** — generate handoff message | `handoff.ps1` | `.augment\skills\orchestration-handoffs\scripts\handoff.ps1 -From "UI Designer" -To "Planner" -ProjectName "{project}" -Findings "decision1","decision2"` |
...
```powershell
.augment\skills\orchestration-artifacts\scripts\check-gate.ps1 -ProjectName "{project}" -Phase "ui-design"
```
