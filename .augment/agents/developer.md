...
| When | Script | Command |
|---|---|---|
| **Phase start** — check upstream planning artifacts | `artifact-status.ps1` | `.augment\skills\orchestration-artifacts\scripts\artifact-status.ps1 -ProjectName "{project}"` |
| **Before handoff** — validate quality gate | `check-gate.ps1` | `.augment\skills\orchestration-artifacts\scripts\check-gate.ps1 -ProjectName "{project}" -Phase "development"` |
| **At handoff** — generate handoff message | `handoff.ps1` | `.augment\skills\orchestration-handoffs\scripts\handoff.ps1 -From "Developer" -To "Code Reviewer" -ProjectName "{project}" -Findings "note1","note2"` |
