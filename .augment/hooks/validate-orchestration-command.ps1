<#
.SYNOPSIS
    Augment PreToolUse hook for orchestration command validation.
    Validates that orchestration skill scripts are invoked with required parameters
    and conform to project safety constraints.

.DESCRIPTION
    This script is called by the Augment hooks system (PreToolUse event)
...
try {
    # Read JSON event data from stdin (Augment hook contract)
    ...
    if ($command -match '\.augment[/\\]skills[/\\]orchestration') {
