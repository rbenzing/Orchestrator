...
3. **Load the Orchestrator role based on chosen mode**:
   - **CLI Mode**: Use `.augment/agents/` subagent configs for spawning multiple parallel agents
     - Verify `.augment/agents/` directory exists
...
**CRITICAL**: When Augment compacts context, the orchestrator loses its in-memory workflow position. A file-based state persistence system prevents this.

### State Persistence Rules

1. **ALWAYS save state at every workflow transition** using:
   ```
   .augment\skills\orchestration-state\scripts\save-state.ps1 -ProjectName "{project}" -Phase "{phase}" -ActiveAgent "{agent}" -NextAction "{what to do next}"
   ```

2. **ALWAYS load state as the FIRST action** in any orchestrator response:
   ```
   .augment\skills\orchestration-state\scripts\load-state.ps1 -ProjectName "{project}"
   ```
...
1. **Load state**: `.augment\skills\orchestration-state\scripts\load-state.ps1 -ProjectName "{project}"`
...
See `.augment/skills/orchestration-state/SKILL.md` for full script documentation.
...
**Agent Files**:
- `.augment/agents/researcher.md` - Research & analysis subagent
- `.augment/agents/architect.md` - Technical architecture subagent
- `.augment/agents/ui-designer.md` - UI/UX specification subagent
- `.augment/agents/planner.md` - Technical planning subagent
- `.augment/agents/developer.md` - Code implementation subagent
- `.augment/agents/code-reviewer.md` - Quality review subagent
- `.augment/agents/tester.md` - Testing & validation subagent
...
Pre-built tools live in `.augment/skills/`. **USE THEM instead of writing ad-hoc commands.**

| Task | DO NOT write this | USE THIS instead |
|------|---------------------|---------------------|
| Search code | `Get-ChildItem -Recurse \| Select-String ...` | `.augment\skills\dev-tools\scripts\grep.ps1 -Pattern "..." -Include "*.js"` |
| Find files | `Get-ChildItem -Recurse -Filter "*.ts"` | `.augment\skills\dev-tools\scripts\find-files.ps1 -Name "*.ts"` |
| Directory tree | `cd path; dir` | `.augment\skills\dev-tools\scripts\tree.ps1 -Path "src" -Depth 3` |
| Git status/history | `git log; git status; git branch` | `.augment\skills\dev-tools\scripts\git-summary.ps1` |
| Git diff | `git diff --stat main..HEAD` | `.augment\skills\dev-tools\scripts\git-diff.ps1 -Ref1 "main" -Stat` |
| Initialize artifacts | Manual `New-Item` for dirs | `.augment\skills\orchestration-artifacts\scripts\init-project.ps1 -ProjectName "my-app"` |
| Artifact dashboard | Manual `Get-ChildItem` on dirs | `.augment\skills\orchestration-artifacts\scripts\artifact-status.ps1 -ProjectName "my-app"` |
| Quality gate check | Manual review | `.augment\skills\orchestration-artifacts\scripts\check-gate.ps1 -ProjectName "my-app" -Phase "research"` |
| Create handoff | Manual file creation | `.augment\skills\orchestration-handoffs\scripts\handoff.ps1 -From "Researcher" -To "Architect" -ProjectName "my-app"` |
| Save workflow state | Manual tracking | `.augment\skills\orchestration-state\scripts\save-state.ps1 -ProjectName "my-app" -Phase "development" -ActiveAgent "Developer" -NextAction "Implement Story #1"` |
| Load workflow state | Manual recall | `.augment\skills\orchestration-state\scripts\load-state.ps1 -ProjectName "my-app"` |
| Run Node.js tests | `cd path; npm test -- --watchAll=false` | `.augment\skills\nodejs-windows\scripts\run-tests.ps1 -ProjectPath "ClientApp"` |
| Run Angular tests | `cmd /c "set NODE_OPTIONS=... && ng test"` | `.augment\skills\angular-windows\scripts\run-tests.ps1 -ProjectPath "ClientApp" -LegacyOpenSSL -Headless` |
| Build Angular | `cmd /c "set NODE_OPTIONS=... && ng build"` | `.augment\skills\angular-windows\scripts\run-build.ps1 -ProjectPath "ClientApp" -LegacyOpenSSL` |
| Serve Angular | `cmd /c "ng serve --port 4200"` | `.augment\skills\angular-windows\scripts\run-serve.ps1 -ProjectPath "ClientApp" -Port 4200` |
| Run .cmd/.bat file | `cmd /c "run-tests.cmd"` | `.\run-tests.cmd` (runs natively in PowerShell) |
| Lint code | `cd path; npm run lint` | `.augment\skills\nodejs-windows\scripts\run-lint.ps1 -ProjectPath "ClientApp"` |
| Lint + auto-fix | `cd path; npm run lint -- --fix` | `.augment\skills\nodejs-windows\scripts\run-lint.ps1 -ProjectPath "ClientApp" -Fix` |
| Build project | `cd path; npm run build` | `.augment\skills\nodejs-windows\scripts\run-build.ps1 -ProjectPath "ClientApp"` |
| Kill process | `Get-NetTCPConnection; Stop-Process` | `.augment\skills\dev-tools\scripts\kill-port.ps1 -Port 3000 -Force` |
| Remove files/dirs | `Remove-Item`, `remove-files` tool, `del` | `.augment\skills\dev-tools\scripts\remove-files.ps1 -Path "dist" -Recurse -Force` |
...
|-------|---------|----------|
| **dev-tools** | grep, find-files, tree, git-summary, git-diff, kill-port, remove-files | `.augment/skills/dev-tools/SKILL.md` |
| **orchestration-artifacts** | init-project, artifact-status, check-gate | `.augment/skills/orchestration-artifacts/SKILL.md` |
| **orchestration-handoffs** | handoff | `.augment/skills/orchestration-handoffs/SKILL.md` |
| **orchestration-state** | save-state, load-state | `.augment/skills/orchestration-state/SKILL.md` |
| **nodejs-windows** | run-tests, run-lint, run-build | `.augment/skills/nodejs-windows/SKILL.md` |
| **angular-windows** | run-tests, run-build, run-serve | `.augment/skills/angular-windows/SKILL.md` |
| **windows-environment** | (reference only) | `.augment/skills/windows-environment/SKILL.md` |
...
Before running ANY npm/npx/node command, read `.augment/skills/nodejs-windows/SKILL.md`. Key rules:
