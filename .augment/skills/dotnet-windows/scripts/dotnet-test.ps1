...
.EXAMPLE
    .augment\skills\dotnet-windows\scripts\dotnet-test.ps1 -ProjectPath "." -Filter "Category=Unit" -NoBuild
#>
[CmdletBinding()]
param(
...
Write-Host ""
Write-Host "  Running .NET tests in: $resolved" -ForegroundColor Cyan
if ($Filter) { Write-Host "  Filter: $Filter" -ForegroundColor Gray }
Write-Host ""
...
try {
    ...
    Write-Host ""
    if ($code -eq 0) { Write-Host "  TESTS PASSED (exit 0, $elapsed)" -ForegroundColor Green }
    else { Write-Host "  TESTS FAILED (exit $code, $elapsed)" -ForegroundColor Red }
    exit $code
