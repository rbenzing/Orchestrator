...
.augment\skills\dotnet-windows\scripts\dotnet-build.ps1 -ProjectPath "." -Configuration "Release" -Verbosity "minimal"
```
Params: `-ProjectPath` (required), `-Configuration`, `-Verbosity`, `-NoRestore`, `-PassThruArgs`

### `dotnet-test.ps1` — Run .NET tests
```
.augment\skills\dotnet-windows\scripts\dotnet-test.ps1 -ProjectPath "tests\MyApi.Tests"
.augment\skills\dotnet-windows\scripts\dotnet-test.ps1 -ProjectPath "." -Filter "FullyQualifiedName~Integration" -NoBuild
```
...
.augment\skills\dotnet-windows\scripts\dotnet-run.ps1 -ProjectPath "src\MyApi" -Configuration "Release" -LaunchProfile "Development"
```
...
```
.augment\skills\dotnet-windows\scripts\dotnet-format.ps1 -ProjectPath "src\MyApi"
.augment\skills\dotnet-windows\scripts\dotnet-format.ps1 -ProjectPath "." -VerifyNoChanges -Severity "warn"
```
