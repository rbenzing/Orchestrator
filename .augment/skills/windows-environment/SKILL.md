...
PowerShell scripts for safe filesystem operations. All scripts block writes to protected directories (`.git`, `.augment`).

### `make-dir.ps1` — Create directories
```
.augment\skills\windows-environment\scripts\make-dir.ps1 -Path "src\components"
.augment\skills\windows-environment\scripts\make-dir.ps1 -Path "src\models","src\services","src\utils"
```
...
```
.augment\skills\windows-environment\scripts\rename-item.ps1 -Path "src\utils.js" -NewName "helpers.js"
.augment\skills\windows-environment\scripts\rename-item.ps1 -Path "src\old-module" -NewName "new-module"
```
...
- All scripts block paths into protected directories (.git, .augment)
...
| Find text in files | `grep -r "pattern" .` | **`.augment\skills\dev-tools\scripts\grep.ps1 -Pattern "pattern"`** |
| Find files | `find . -name "*.js"` | **`.augment\skills\dev-tools\scripts\find-files.ps1 -Name "*.js"`** |
| Directory tree | `tree` / `ls -R` | **`.augment\skills\dev-tools\scripts\tree.ps1 -Path "src" -Depth 3`** |
| Git status | `git log; git status` | **`.augment\skills\dev-tools\scripts\git-summary.ps1`** |
| Git diff | `git diff --stat` | **`.augment\skills\dev-tools\scripts\git-diff.ps1 -Stat`** |
| Kill process/port | `kill -9 PID` | **`.augment\skills\dev-tools\scripts\kill-port.ps1 -Port 3000 -Force`** |
...
> **IMPORTANT**: For tasks with **bold** entries above, ALWAYS use the toolkit script instead of raw PowerShell commands. See `.augment/skills/dev-tools/SKILL.md` and `.augment/skills/nodejs-windows/SKILL.md` for full parameter details.
...
# ✅ RIGHT:
.augment\skills\dev-tools\scripts\find-files.ps1 -Name "*.js" -Path "src\__tests__"

# ❌ WRONG: find . -name "*.test.js" -type f
# ✅ RIGHT:
.augment\skills\dev-tools\scripts\find-files.ps1 -Name "*.test.js"

# ❌ WRONG: grep -r "import" src/
# ❌ ALSO WRONG: Select-String -Path "src\**\*.js" -Pattern "import" -Recurse
# ✅ RIGHT:
.augment\skills\dev-tools\scripts\grep.ps1 -Pattern "import" -Path "src" -Include "*.js"

# ❌ WRONG: cmd /c "dir /s /b" to list files
# ✅ RIGHT:
.augment\skills\dev-tools\scripts\tree.ps1 -Path "." -ShowFiles
...
# ✅ RIGHT:
.augment\skills\angular-windows\scripts\run-tests.ps1 -ProjectPath "C:\app" -LegacyOpenSSL -Headless
...
# ✅ RIGHT:
.augment\skills\angular-windows\scripts\run-build.ps1 -ProjectPath "ClientApp" -Configuration "production"
```
...
$name = "user-auth"; .augment\skills\orchestration-artifacts\scripts\init-project.ps1 -ProjectName $name

❌ WRONG — $env variables also get stripped:
Write-Host $env:TEMP

✅ RIGHT — always use literal values directly:
.augment\skills\orchestration-artifacts\scripts\init-project.ps1 -ProjectName "user-auth"
...
.augment\skills\dev-tools\scripts\find-files.ps1 -Name "*.js"
.augment\skills\dev-tools\scripts\find-files.ps1 -Name "*.jsx" -Path "src"

# Search file contents (replaces: grep -r "pattern" ., Select-String -Recurse)
.augment\skills\dev-tools\scripts\grep.ps1 -Pattern "pattern" -Include "*.js"
.augment\skills\dev-tools\scripts\grep.ps1 -Pattern "TODO|FIXME" -Path "src" -Context 2

# Directory listing (replaces: dir, Get-ChildItem, tree)
.augment\skills\dev-tools\scripts\tree.ps1 -Path "src" -Depth 3 -ShowFiles

# Git operations (replaces: git log, git status, git diff)
.augment\skills\dev-tools\scripts\git-summary.ps1
.augment\skills\dev-tools\scripts\git-diff.ps1 -Stat
