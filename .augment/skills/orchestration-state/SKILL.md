...
File-based state persistence for the orchestrator. After Augment compacts context, the orchestrator reads the state file to recover its exact workflow position.
...
.augment\skills\orchestration-state\scripts\save-state.ps1 -ProjectName "user-auth" -Phase "reviews" -ActiveAgent "Code Reviewer" -CurrentStory "Story #1: User Login" -StoryStatus "review" -CompletedStories "" -StoryQueue "Story #2,Story #3" -NextAction "Code Reviewer reviewing Story #1"
```
...
```
.augment\skills\orchestration-state\scripts\load-state.ps1 -ProjectName "user-auth"
```
